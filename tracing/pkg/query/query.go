...
func Values(v interface{}) (url.Values, []string, error) {
	...
	if val.Kind() != reflect.Struct {
		return nil, fieldNames, fmt.Errorf("query: Values() expects struct input. Got %v", val.Kind())
	}

	err := reflectValue(values, val, "", &fieldNames)
	return values, fieldNames, err
}

// reflectValue populates the values parameter from the struct fields in val.
// Embedded structs are followed recursively (using the rules defined in the
// Values function documentation) breadth-first.
func reflectValue(values url.Values, val reflect.Value, scope string, fieldNames *[]string) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		*fieldNames = append(*fieldNames, sf.Name)
		...
		if sv.Kind() == reflect.Struct {
			if err := reflectValue(values, sv, name, fieldNames); err != nil {
				return err
			}
			continue
		}
	...
	for _, f := range embedded {
		if err := reflectValue(values, f, scope, fieldNames); err != nil {
			return err
		}
	}
